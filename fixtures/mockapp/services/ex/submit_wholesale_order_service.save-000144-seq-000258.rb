...
class SubmitWholesaleOrderService
  ...
  Request = Struct.new(:raw_request) do
    ...
    alias ordering_status_approved? account_approved_to_order?

    def raw_requested_by = raw_request.requested_by
    def requested_by = raw_requested_by.to_s.strip.downcase

    def raw_buyers = account.fetch(:buyers)
    def buyers = raw_buyers.map(&:downcase)
    def requester_is_buyer? = buyers.include?(requested_by)
    alias requester_is_allowed? requester_is_buyer?

    def raw_destination = raw_request.destination
    def destination = @destination ||= raw_destination.transform_values { |value| value.to_s.strip }

    def raw_state = destination[:state]
    def state = raw_state.upcase

    def allowed_states = account.fetch(:allowed_states)
    def supported_destination? = allowed_states.include?(state)
    alias state_in_accounts_allowed_states? supported_destination?
  end
  ...
  def call
    store.transaction do
      ...
      raise ArgumentError, "requester is not allowed" unless request.requester_is_buyer?
      raise ArgumentError, "unsupported destination" unless request.supported_destination?
  ...
  # now...
  # - we're protected from change
  # - no tight coupling on hash keys
  # - calling a method (.) is easier than ([:...])
  # - we're providing a lot more information to whoever calls this service
  # - order retains it's full context
  # - lazy evaluation - only process what the caller wants
  Response = Struct.new(:order, :units, :volume_discount_cents, :shipping_cents, :tax_cents, :total_cents) do
    def order_number = order.fetch(:number)
    def company_name = order.fetch(:company_name)
    def status = order.fetch(:status)

    def units = unit_count
    def discount = money(volume_discount_cents)
    def shipping = money(shipping_cents)

    def tax = money(tax_cents)
    def total = money(total_cents)
  end
