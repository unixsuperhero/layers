...
class SubmitWholesaleOrderService
  ...
  Request = Struct.new(:raw_request) do
    ...
    def state = destination[:state]
    def postal_code = destination[:postal_code]
  end
  ...
  def call
    store.transaction do
      ...
      request.destination[:state] = request.destination.fetch(:state).upcase
      request.destination[:postal_code] = request.destination.fetch(:postal_code).gsub(/\s+/, "")
      raise ArgumentError, "unsupported destination" unless account.fetch(:allowed_states).include?(request.destination.fetch(:state))
