...
class SubmitWholesaleOrderService
  ...
  def call
    store.transaction do
      ...
      # == forgot to update all refs to account ==
      raise ArgumentError, "requester is not allowed" unless request.account.fetch(:buyers).map(&:downcase).include?(request.requested_by)
