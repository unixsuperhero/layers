...
class SubmitWholesaleOrderService
  ...
  Request = Struct.new(:raw_request) do
    ...
    def raw_postal_code = destination[:postal_code]
    def postal_code = raw_postal_code.gsub(/\s+/, "" w
  end
  ...
  def call
    store.transaction do
      ...
      request.state = request.state.upcase
      request.postal_code = request.postal_code.gsub(/\s+/, "")
      raise ArgumentError, "unsupported destination" unless account.fetch(:allowed_states).include?(request.state)
