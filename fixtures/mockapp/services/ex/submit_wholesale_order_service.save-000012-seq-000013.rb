...
class SubmitWholesaleOrderService
  ...
  def request = Request.new(raw_request)

  def call
    store.transaction do
      account_number = @raw_request.account_number.to_s.strip.upcase
      account = store.accounts.fetch(account_number)
      raise ArgumentError, "account cannot place orders" unless account.fetch(:ordering_status) == "approved"

      requested_by = @raw_request.requested_by.to_s.strip.downcase
      raise ArgumentError, "requester is not allowed" unless account.fetch(:buyers).map(&:downcase).include?(requested_by)

      destination = @raw_request.destination.transform_values { |value| value.to_s.strip }
      ...
      line_items = []
      @raw_request.line_items.each do |requested_item|
      ...
      delivery_speed = @raw_request.delivery_speed.to_s.strip.downcase
      ...
      order = {
        ...
        submitted_at: Time.parse(@raw_request.submitted_at.to_s),
