...
// `control` (newer analyzers put the keyword in `data.name`; older docs fall back to the token text).
export function effectLabel(mark, text) {
  ...
  switch (data.kind) {
    ...
    case "io":
      return `io: ${data.what}`;
    case "control":
      return data.name ?? text ? `control flow: ${data.name ?? text}` : "control flow";
    case "calls":
