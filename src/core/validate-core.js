...
// Browser-safe: the caller supplies the schema (Node reads it from disk, Vite imports the JSON).
export function createValidator(schema) {
  const validateSchema = new Ajv({ allErrors: true }).compile(schema);
  return (doc) => validateDoc(validateSchema, doc);
}

function validateDoc(validateSchema, doc) {
